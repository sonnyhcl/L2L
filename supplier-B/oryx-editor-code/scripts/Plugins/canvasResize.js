...
ORYX.Plugins.CanvasResizeButton = Clazz.extend({
	...
	construct: function(canvas, position, callback){
		...
		// a boolean if the current mouse vessel.event 
		...
		//scrollNode.addEventListener(		ORYX.CONFIG.EVENT_MOUSEOUT, 	function(vessel.event){button.hide()}, true )
		parentNode.parentNode.addEventListener(	ORYX.CONFIG.EVENT_MOUSEOUT, 	function(event){hideButtons()} , true );
		//svgRootNode.addEventListener(	ORYX.CONFIG.EVENT_MOUSEOUT, 	function(vessel.event){ inCanvas = false } , true );
