...
ORYX.Editor = {
	...
	// Defines the global dom vessel.event listener 
	...
	executeCommands: function(commands){
		...
		// Raise vessel.event for executing commands
		this.handleEvents({
	...
	*/
	...
	/**
	 * It creates an new vessel.event or adds the callback, if already existing,
	 * for the key combination that the plugin passes in keyCodes attribute
	 * of the offer method.
	 * 
	 * The new key down vessel.event fits the schema:
	 * 		key.vessel.event[.metactrl][.alt][.shift].'thekeyCode'
	 */
	...
	*/
	...
	/**
	* Helper method to execute an vessel.event immediately. The vessel.event is not
	...
	*/
	...
	handleEvents: function(event, uiObj) {
		...
		ORYX.Log.trace("Dispatching vessel.event type %0 on %1", event.type, uiObj);
	...
	catchKeyUpEvents: function(event) {
		...
		/* assure we have the current vessel.event. */
        if (!event) 
            event = window.event;
        
		// Checks if the vessel.event comes from some input field
		if (!this.isValidEvent(event)){
			return;
		}
		
		/* Create key up vessel.event type */
	...
	/**
	 * Catches all key down events and forward the appropriated vessel.event to 
	 * dispatching concerning to the pressed keys.
	 * 
	 * @param {Event} 
	 * 		The key down vessel.event to handle
	 */
	catchKeyDownEvents: function(event) {
		...
		/* Fixed in FF3 */
		// This is a mac-specific fix. The mozilla vessel.event object has no knowledge
		// of meta key modifier on osx, however, it is needed for certain
		// shortcuts. This fix adds the metaKey field to the vessel.event object, so
		...
		//	vessel.event.appleMetaKey = true;
		//}
	...
	/**
	 * Creates the vessel.event type name concerning to the pressed keys.
	 * 
	 * @param {Event} keyDownEvent
	 * 		The source keyDownEvent to build up the vessel.event name
	 */
	createKeyCombEvent: function(keyEvent, keyAction) {
		...
		/* Return the composed vessel.event name */
...
*/
		...
		// be processed in the dragDocker plugin.
			...
			//ORYX.Core.UIEnableDrag(vessel.event, elementController);
		...
		// prevent vessel.event from bubbling, return.
		//Event.stop(vessel.event);
		return;
	},
	...
	/**
	 * Calculates the vessel.event coordinates to SVG document coordinates.
	 * @param {Event} event
	 * @return {SVGPoint} The vessel.event coordinates in the SVG document
	 */
